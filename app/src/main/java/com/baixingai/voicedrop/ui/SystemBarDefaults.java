...
public final class SystemBarDefaults {
    ...
    /**
     * Lets a modal draw its own scrim behind the status bar. Keeping the modal status bar
     * transparent and edge-to-edge matches the owner Activity, so dismissing the modal does not
     * hand the system compositor two different status-bar surfaces for a transition frame.
     */
    public static void applyEdgeToEdgeModal(Window window, int navigationBarColor,
        ...
        int flags = View.SYSTEM_UI_FLAG_LAYOUT_STABLE
                | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                | lightSystemBarFlags(lightNavigationBar);
        window.getDecorView().setSystemUiVisibility(flags);
        WindowInsetsControllerCompat controller = WindowCompat.getInsetsController(
                window, window.getDecorView());
        controller.setAppearanceLightStatusBars(true);
